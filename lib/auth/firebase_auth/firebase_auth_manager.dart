...
class FirebaseAuthManager extends AuthManager
    ...
    try {
      ...
      if (e.code == 'requires-recent-login') {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Wyloguj i zaloguj się')),
        );
      }
    }
  }
  ...
  Future resetPassword({
    ...
    try {
      ...
      ScaffoldMessenger.of(context).hideCurrentSnackBar();
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Błąd podczas logowania')),
      );
      return null;
    }
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
          content:
              Text('Link resetujący hasło został wysłany na twoje emaila')),
    );
  }
    ...
    phoneAuthManager.addListener(() {
      ...
      if (phoneAuthManager.triggerOnCodeSent) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text('Błąd podczas logowania'),
        ));
  ...
  Future<BaseAuthUser?> _signInOrCreateAccount(
    ...
    try {
      ...
      final errorMsg = switch (e.code) {
        ...
        'INVALID_LOGIN_CREDENTIALS' => 'Wprowadzono złe dane przy logowaniu',
        _ => 'Błąd podczas logowania',
      };
      ScaffoldMessenger.of(context).hideCurrentSnackBar();
      ScaffoldMessenger.of(context).showSnackBar(
